...
function WordCard({words, deleter, audioHandler, baseUrl, addWordToUser}:any): JSX.Element {
  ...
  const mappedWords = words.map((el:IWord, i: number, arr: never[]): JSX.Element => (
    <div key={arr.length - i} className="card mx-3 mb-3 pb-3" style={{ width: '12rem' }}>
      ...
      <div className='Col'>
        <Button title="добавить в сложные" variant="outline-info" onClick = {()=>addWordToUser(el.id)}><LightningFill color="yellow" size={20} /></Button>
        <Button title="удалить из списка" variant="outline-info" onClick={() => deleter(el.id)}><X color="red" size={20} /></Button>
