...
const wordTypeProps:IWordType = {
  'learn': { 'isLearn': false, 'isHard': false, 'isDeleted': false },
  'hard': { 'isLearn': false, 'isHard': true, 'isDeleted': false },
  'delete': { 'isLearn': false, 'isHard': false, 'isDeleted': true }
}

function WordCard({words,audioHandler, baseUrl, addWordToUser, updateUserWord, isAuth}:any): JSX.Element {
  ...
  const mappedWords = words.filter(((el:IWord)=>{
    ...
    return  <div key={arr.length - i} className="card mx-3 mb-3 pb-3 animated" style={{backgroundColor:'transparent', border:'none'}}>
      ...
      <div className='front' style={{backgroundColor:(el.hasOwnProperty('userWord')&&el.userWord.optional.isHard?'rgba(255, 173, 173,0.9)':'rgba(253, 255, 182,0.9)')}}>
      ...
      <div className='back' style={{backgroundColor:(el.hasOwnProperty('userWord')&&el.userWord.optional.isHard?'rgba(255, 173, 173,0.9)':'rgba(253, 255, 182,0.9)')}}>
      ...
      <div className='Col' style={{position:'absolute', bottom:'5px', left:'0', right:'0'}}>
        {isAuth && (<>
        <Button title="добавить в сложные" disabled = {el.hasOwnProperty('userWord')&&el.userWord.optional.isHard?true:false} variant="outline-info"
